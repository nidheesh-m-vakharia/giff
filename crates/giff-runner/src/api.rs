...
async fn get_stack(State(s): State<ApiState>, Path(id): Path<String>) -> impl IntoResponse {
    let pulls = match s.db.list_all_pulls().await {
        Ok(v) => v,
        Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, format!("{}", e)).into_response(),
    };
...
async fn list_events(State(s): State<ApiState>, Query(q): Query<EventQuery>) -> impl IntoResponse {
